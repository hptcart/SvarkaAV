Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro Geometric Dissolve/d8afab38923163e4581e0d60a5c19ca6"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void GeometryProgram(triangle VertexOut p[3], inout TriangleStream<VertexOut> triStream)
			{
				...
				if ((0.0 /*_GeoDissolveGradientType*/) == GEO_DISSOLVE_TEXTURE)
				{
					geoDissolveAlpha = (1 - tex2Dlod(_GeoDissolveGradient, float4(poiUV(uv, float4(5,5,0,0)) + float4(1,1,0,0) * _Time.x, 0, 0)).r) * .5 ;
					geoDissolveAlpha = -1 + geoDissolveAlpha + _GeoDissolveAlpha * 2;
					p[0].geoDissolveAlpha = remapClamped( - .5, 1 / (0.528 /*_GeoDissolveExpansionTime*/), geoDissolveAlpha);
					p[1].geoDissolveAlpha = remapClamped( - .5, 1 / (0.528 /*_GeoDissolveExpansionTime*/), geoDissolveAlpha);
					p[2].geoDissolveAlpha = remapClamped( - .5, 1 / (0.528 /*_GeoDissolveExpansionTime*/), geoDissolveAlpha);
				}
				else if ((0.0 /*_GeoDissolveGradientType*/) == GEO_DISSOLVE_P2P)
				{
					float3 pos = center;
					if ((0.0 /*_GeoDissolveVertexColor*/))
					{
						pos = vertexColorCenter;
					}
					float3 direction = normalize(float4(0,0,0,1).xyz - float4(0,0,16.33,1).xyz);
					float3 currentPos = lerp(float4(0,0,16.33,1).xyz, float4(0,0,0,1).xyz, _GeoDissolveAlpha);
					float distanceTo = dot(currentPos - pos, direction);
					geoDissolveAlpha = distanceTo;
					p[0].geoDissolveAlpha = geoDissolveAlpha / (0.528 /*_GeoDissolveExpansionTime*/);
					p[1].geoDissolveAlpha = geoDissolveAlpha / (0.528 /*_GeoDissolveExpansionTime*/);
					p[2].geoDissolveAlpha = geoDissolveAlpha / (0.528 /*_GeoDissolveExpansionTime*/);
				}
				else if ((0.0 /*_GeoDissolveGradientType*/) == GEO_DISSOLVE_SPHERE)
				{
				}
				else if ((0.0 /*_GeoDissolveGradientType*/) == GEO_DISSOLVE_CENTER_OUT)
				{
				}
				else if ((0.0 /*_GeoDissolveGradientType*/) == GEO_DISSOLVE_OUT_IN)
				{
				}
				geoDissolveAlpha = saturate(geoDissolveAlpha) / (0.528 /*_GeoDissolveExpansionTime*/);
				...
				for (int i = 0; i < 3; i++)
				{
					p[i].localPos.xyz = lerp(p[i].localPos.xyz, center, remapClamped(float4(0.1,1,0,0).x, float4(0.1,1,0,0).y, geoDissolveAlpha));
					if ((1.0 /*_GeoDissolveSpiral*/))
					{
						p[i].localPos.xyz += geoDissolveAlpha * (3.85 /*_GeoDissolveTravelDistance*/) * float3(sin(geoDissolveAlpha * 6.28318530718 * float4(0,9.3,-4.76,0.22).x * float4(0,9.3,-4.76,0.22).w + atan2(outDir.z, outDir.x)), sin(geoDissolveAlpha * 6.28318530718 * float4(0,9.3,-4.76,0.22).y * float4(0,9.3,-4.76,0.22).w + atan2(outDir.z, outDir.x)), cos(geoDissolveAlpha * 6.28318530718 * float4(0,9.3,-4.76,0.22).z * float4(0,9.3,-4.76,0.22).w + atan2(outDir.z, outDir.x)));
					}
					else
					{
						p[i].localPos.xyz += outDir * geoDissolveAlpha * (3.85 /*_GeoDissolveTravelDistance*/);
					}
			...
			{
				float4 geoDissolveColor = float4(poiThemeColor(poiMods, float4(1,0.03705543,0.05257591,1).rgb, (0.0 /*_GeoDissolveColorThemeIndex*/)), float4(1,0.03705543,0.05257591,1).a);
				#if defined(PROP_GEODISSOLVETEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				poiFragData.baseColor = lerp(poiFragData.baseColor, geoDissolveColor.rgb, smoothstep(float4(0,1,0,0).x, float4(0,1,0,0).y, geoDissolveAlpha * geoDissolveColor.a));
				poiFragData.emission += smoothstep(float4(0,1,0,0).x, float4(0,1,0,0).y, geoDissolveAlpha) * geoDissolveColor.rgb * (1.0 /*_geoEmissionStrength*/) * geoDissolveColor.a;
			}
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_EmissionMapUV*/)], float4(5,5,0,0)), float4(1,1,0,0)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(1,0,0,1).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(5,5,0,0).xy) + _Time.x * (5.0 /*_EmissionCenterOutSpeed*/)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(1,0,0,1).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				}
				#else
				...
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(0,5,0,0), (0.0 /*_EmissionAL0StrengthBand*/), float4(1,1,0,0), (0.0 /*_EmissionAL0MultipliersBand*/), (1.0 /*_EmissionAL0Enabled*/));
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor = saturate(poiFragData.baseColor + (0.63 /*_MainBrightness*/) * hueShiftAlpha.g);
				#endif

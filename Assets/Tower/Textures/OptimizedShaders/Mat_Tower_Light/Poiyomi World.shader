Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/3bebec839e7db00469f44d555b22581e"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				+ (0.0 /*_DissolveAlpha8*/)
				+ (0.0 /*_DissolveAlpha9*/);
				float dds = (0.658 /*_DissolveDetailStrength*/);
				...
				#if defined(PROP_DISSOLVEDETAILNOISE) || !defined(OPTIMIZER_ENABLED)
				float dissolveDetailNoise = POI2D_SAMPLER_PAN(_DissolveDetailNoise, _MainTex, poiUV(poiMesh.uv[(0.0 /*_DissolveDetailNoiseUV*/)], float4(100,100,0,0)), float4(5,5,0,0));
				#else
				...
				switch((2.0 /*_DissolveType*/))
				{
					...
					case 2: // Point to Point
					{
						...
						float3 currentPos;
						float distanceTo = 0;
						direction = normalize(float4(0,-1,0,0) - float4(0,150,0,0));
						currentPos = lerp(float4(0,150,0,0), float4(0,-1,0,0), dissolveAlpha);
				...
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(0.6070304,0,0,1).rgb, (0.0 /*_DissolveEdgeColorThemeIndex*/)), float4(0.6070304,0,0,1).a);
					#ifndef POI_SHADOW
				...
				poiFragData.emission += lerp(0, dissolveToTexture * (0.0 /*_DissolveToEmissionStrength*/), dissolveAlpha) + lerp(0, edgeColor.rgb * (10.0 /*_DissolveEdgeEmission*/), smoothstep(0, 1 - (0.0 /*_DissolveEdgeHardness*/) * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				clip(poiFragData.alpha - (0.203 /*_Cutoff*/));
